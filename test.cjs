...
async function runTests() {
  ...
  const browser = await puppeteer.launch({
    ...
    headless: "new",
    args: ['--window-size=800,600', '--disable-background-timer-throttling', '--disable-backgrounding-occluded-windows', '--disable-renderer-backgrounding']
  });
  ...
  try {
    ...
    // Wait for players to join properly (button enabled)
    console.log("Waiting for players to sync and button to be enabled...");
    await pages[0].waitForFunction(() => {
        const btns = document.querySelectorAll('button');
        for (let b of btns) {
            if (b.innerText.toUpperCase().includes('LANCER LA PARTIE') && !b.disabled) return true;
        }
        return false;
    }, {timeout: 15000});
